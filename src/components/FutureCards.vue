    ...
    <div class="screen" v-for="(screen, index) in screensData" :key="index" @mouseenter="animateText(index)">
      ...
      <div class="screen-content">
        ...
        <div class="screen-user">
          <span class="name" :data-value="screen.name">{{ text[index].value }}</span>
          <a class="link" :href="screen.link" rel="noopener noreferrer">
            SEE MORE
          </a>
        </div>
      </div>
  ...
  setup() {
    ...
    const animateText = (index) => {
      ...
      const interval = setInterval(() => {
        text[index].value = text[index].value
          .split('')
          .map((letter, index) =>
            index < iteration ? screen.name[index] : letters[Math.floor(Math.random() * 26)]
          )
          .join('');

        if (iteration >= screen.name.length) {
